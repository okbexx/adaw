...
| AC-Z-1 | productization | 作为用户，我运行 adaw skill export 后，能得到可放入 Codex Skills 的 ADAW 使用说明。 | passing | verified | test-summary: adaw skill export emits a Codex Skill draft covering setup, doctor, destructive install preview, brainstorm, draft, approve, criterion update, profile, resume, next, evidence, evaluate, status, and report. |
