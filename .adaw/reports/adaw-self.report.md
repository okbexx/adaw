...
| AC-Z-1 | productization | 作为用户，我运行 adaw skill export 后，能得到可放入 Codex Skills 的 ADAW 使用说明。 | passing | verified | test-summary: adaw skill export emits a Codex Skill draft covering setup, doctor, brainstorm, draft, approve, criterion update, profile, resume, next, evidence, evaluate, status, and report. |
| AC-Z-2 | productization | 作为用户，我运行 adaw install 后，能把 ADAW 放入当前项目的可用入口，并且不会意外覆盖已有内容。 | passing | verified | test-summary: adaw install creates .adaw assets, optional repo-local Skill, and manifest while skipping existing content by default. |
...
| AC-Z-7 | productization | 作为用户，我运行 adaw install 后，能看到当前项目的 ADAW 接入登记信息，并判断版本、托管入口、active goals 和 Skill 状态是否可信。 | passing | verified | test-summary: install dry-run and install output expose create/skip/update actions and .adaw/manifest.json records version, managed files, active goals, Skill state, and capabilities. |
