...
| AC-P-12 | protocol | 作为用户，我运行 adaw check 后，能发现 AC 是否缺少真实用户操作或可观察结果，而不是只检查是否以“作为用户”开头。 | 对包含“测试通过”“字段存在”等内部完成条件的验收草案运行 adaw check，再对包含真实用户操作和报告结果的草案运行 adaw check。 | 内部完成条件会被拒绝；包含用户运行、打开、查看、确认等操作，并说明用户能看到或判断什么结果的 AC 可以通过。 | passing |
| AC-P-13 | protocol | 作为用户，我打开 adaw report 后，能先看到完成结论、当前缺口和是否需要我介入，再查看详细 AC 表格。 | 运行 adaw report 并阅读报告顶部内容，再向下查看 Acceptance Status 表格。 | 报告在详细表格前显示 Decision Summary，包含 completion、current gap、user intervention 和 workflow status；blocked 报告也先显示需要用户采取的动作。 | passing |
