...
| AC-Z-1 | productization | 作为用户，我获取 OpenNori 包后，我的 agent 能通过 Codex Plugin 发现 OpenNori Skills，而不需要我运行 skill export 或记住内部 Skill 名。 | 查看 .codex-plugin/plugin.json、skills/nori*/SKILL.md、README 和 opennori doctor 输出。 | 包内包含 .codex-plugin/plugin.json 且 skills 指向 ./skills/；nori 总入口和子 Skills 说明自然语言路由；doctor 报告 plugin_manifest、plugin_skills 和 manifest_plugin_state；不存在 opennori skill export 作为用户路径。 | passing |
...
| AC-A-9 | architecture | 作为用户，我打开 README 或官网时，能理解 OpenNori 同时提供产品验收、架构基线、Plugin Skills、证据和完成判断，而不是只看到旧的验收工具或 Skill Pack 安装心智。 | 阅读 README、官网首页、protocol 和 OpenNori Skills。 | 用户能看到 Architecture Baseline、Architecture Challenge、build-vs-buy、Plugin Skills、项目 profile 和 Product AC 分离的说明与用例。 | passing |
| AC-A-10 | architecture | 作为用户，我能在一个非 OpenNori 核心仓库里，按新的 Plugin-first 边界完整跑通自然语言目标、Product AC、Architecture Baseline、执行、证据、报告和完成判断。 | 选择一个非 OpenNori 核心仓库，使用当前 OpenNori 从目标到报告跑完；过程中不得使用 opennori skill export、install --skill、refresh-skill 或项目内 OpenNori Skill copy/sync。 | 报告同时呈现产品完成状态、架构健康状态、Plugin Skills/package 能力、证据、未解决风险和是否需要用户确认；用户不需要记住 CLI 参数；doctor/check 不依赖项目内 OpenNori Skill Pack。 | passing |
