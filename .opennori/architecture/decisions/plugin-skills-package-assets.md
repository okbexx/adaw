...
Use the OpenAI/Codex Skill directory shape as the source of truth: plugins/opennori/skills/<name>/SKILL.md with YAML frontmatter, exposed through plugins/opennori/.codex-plugin/plugin.json and the repo marketplace, then read by a thin runtime module for Plugin state, manifest, and doctor checks.

## Candidates Checked

- Current project: OpenNori ships .agents/plugins/marketplace.json, plugins/opennori/.codex-plugin/plugin.json, and package-local plugins/opennori/skills/*/SKILL.md assets; src/skills.ts reads package assets and src/plugin.ts reports Plugin state without copying Skills into user projects.
